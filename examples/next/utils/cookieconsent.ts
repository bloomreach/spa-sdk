...
const projectToken = process.env.NEXT_PUBLIC_EXPONEA_PROJECT_TOKEN ?? '';
const apiUrl = process.env.NEXT_PUBLIC_EXPONEA_API_URL ?? '';
...
export const runPersonalization = (path = '/'): void => {
  initializePersonalization({ projectToken, targetURL: apiUrl, path });
};

const runExponeaCookie = (path: string): void => {
  injectScript(exponeaSdkSnippet);
